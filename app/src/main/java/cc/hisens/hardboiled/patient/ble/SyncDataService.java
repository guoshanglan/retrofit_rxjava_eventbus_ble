...
//        for (Integer a:mLostPacketId){
//            Log.e("diushipackId",a+"");
//        }
        Log.e("diushi",mLostPacketId.size()+"");
        Log.e("chongchuan",mRetryTimes+"");
...
//    }
    ...
    private void saveEdInfos() {
        ...
        if (edInfos != null && edInfos.size() > 0) {
            ...
            mEndSleep = edInfos.get(edInfos.size() - 1).getStartTime()+5000;
        }
        ...
        if (edInfos.size() > 0) {
            ...
            KLog.i(TAG, "-->> 算法分析出错" + edInfos.size() );
