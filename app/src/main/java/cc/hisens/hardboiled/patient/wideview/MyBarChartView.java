...
public class MyBarChartView extends SurfaceView {

    private int barInterval; //柱子之间的间距
    private int barWidth;   //柱子宽度
    private int top_text_size;  //顶部文字大小，不过目前没有绘制顶部文字
    ...
    private int defaultHeight = dp2Px(145);
    private int bottom_view_height = dp2Px(0);
    private int top_text_height = dp2Px(30);
    private float scaleTimes = 1;
    private float lastX = 0;
    private float lastY = 0;
    private int defaultScaleHeight=450;  //默认的伸缩模式
    ...
    public void setBarChartData(ArrayList<BarData> innerData) {
        ...
        scaleTimes = (float)defaultScaleHeight / (float) (defaultHeight - bottom_view_height - top_text_height);
        invalidate();
    }
...
////                //绘制bar
                ...
                //绘制下面的文字
                float bottomTextWidth = mBottomTextPaint.measureText(innerData.get(i).getBottomText());
            ...
            //绘制虚线的y坐标
            if(scaleTimes != 0){
                start160=(defaultHeight - bottom_view_height)-90/scaleTimes;
                start270=(defaultHeight - bottom_view_height)-140/scaleTimes;
                start450=(defaultHeight - bottom_view_height)-190/scaleTimes;
                start550=(defaultHeight - bottom_view_height)-550/scaleTimes;

            }
            drawBottomDashLine1(canvas,start550,start450,start270,start160);
        }

    }

    private void drawBottomLine(Canvas canvas) {
        canvas.drawLine(paddingLeft, defaultHeight - bottom_view_height, 800, defaultHeight - bottom_view_height, mBottomLinePaint);
    }

    //画虚线
    private void drawBottomDashLine1(Canvas canvas, float start550, float start450, float start270, float start160) {
        ...
        path.moveTo(0,  start550);
        path.lineTo(measureWidth,   start550);
        canvas.drawPath(path, paint);

        canvas.drawText("550", 15, start550+10, paint2);


        path.moveTo(0, start450);
        path.lineTo(measureWidth,start450);
        canvas.drawPath(path, paint);
        canvas.drawText("190", 15, start450+10, paint2);

        path.moveTo(0, start270);
        path.lineTo(measureWidth,start270);
        canvas.drawPath(path, paint);
        canvas.drawText("140", 15, start270+10, paint2);

        path.moveTo(0, start160);
        path.lineTo(measureWidth,start160);
        canvas.drawPath(path, paint);
        canvas.drawText("90", 15, start160+10, paint2);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        paddingLeft = getPaddingLeft()+dp2Px(30);
