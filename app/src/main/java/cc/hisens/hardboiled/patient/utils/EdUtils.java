...
public class EdUtils {
    ...
    public static Ed getEd(List<EdInfo> edInfoList, long startTimestamp, long endTimestamp) {
        ...
        for (EdInfo info : edInfoList) {
            ...
            total+=info.getErectileStrength();
        ...
        ed.setSync(false);
        ed.setAverage(total/edInfoList.size());
...
//        test();
    ...
    public static List<Ed> initRealm() {
        ...
        long startTime = 1524320660L;
        long endTime = 1524350660L;
        List<Ed>edList=new ArrayList<>();
        for (int j = 0; j < 25; j++) {
            Ed ed = new Ed();
            ...
            for (int i = 0; i < 6; i++) {
                ...
                edInfo.setErectileStrength((int) (Math.random() * 300) + 20);
